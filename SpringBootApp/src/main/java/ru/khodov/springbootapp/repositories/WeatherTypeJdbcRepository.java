...
public class WeatherTypeJdbcRepository {
    ...
    public List<WeatherType> getAllWeatherTypes() {
        return jdbcTemplate.query("SELECT * FROM weather_type", new BeanPropertyRowMapper<>(WeatherType.class));
    }

    public WeatherType getWeatherTypeById(Long id) {
        return jdbcTemplate.queryForObject("SELECT * FROM weather_type WHERE id = ?", new BeanPropertyRowMapper<>(WeatherType.class), id);
    }
